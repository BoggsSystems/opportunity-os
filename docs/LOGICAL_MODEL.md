...
### `daily_command_queues`

Purpose: durable daily execution plans for Today's Command Queue. A queue is the cross-cycle action surface the Conductor uses to guide a user through the day.
...
* `status` `daily_command_queue_status` NOT NULL DEFAULT `planned`
* `title` VARCHAR NULL
* `summary` TEXT NULL
* `target_action_count` INTEGER NULL
...
* unique composite index on (`user_id`, `queue_date`)
* index on `user_id`
* index on `queue_date`
* index on `status`
* composite index on (`user_id`, `status`, `queue_date`)

#### Notes

* A queue represents the daily execution plan, not the strategy cycle.
* A user may jump across many Campaigns, ActionLanes, and ActionCycles while executing one queue.
* The queue can be regenerated or reprioritized as replies arrive, connector syncs complete, or user behavior changes the day.
* `metadata_json` stores queue-generation context such as scoring inputs, excluded actions, daily constraints, or Conductor summary.

---

### `command_queue_items`

Purpose: prioritized slots inside Today's Command Queue. These are the one-at-a-time items the Conductor presents to the user.

#### Columns

* `id` UUID PK
* `command_queue_id` UUID NOT NULL FK -> `daily_command_queues.id`
* `user_id` UUID NOT NULL FK -> `users.id`
* `offering_id` UUID NULL FK -> `offerings.id`
* `campaign_id` UUID NULL FK -> `campaigns.id`
...
* `priority_score` INTEGER NOT NULL DEFAULT 50
* `status` `command_queue_item_status` NOT NULL DEFAULT `queued`
* `title` VARCHAR NOT NULL
* `reason` TEXT NULL
...
* unique composite index on (`command_queue_id`, `position`)
* index on `command_queue_id`
* index on `user_id`
* index on `offering_id`
* index on `campaign_id`
* index on `action_lane_id`
...
* Most queue items should reference an ActionItem, which remains the concrete work-unit system of record.
* Offering, Campaign, ActionLane, and ActionCycle references are optional denormalized pointers for fast queue rendering, filtering, and admin analytics.
* `position` controls the Conductor's presentation order within a specific daily queue.
* `reason` explains why the AI selected the action now.
* Status tracks the daily execution lifecycle: queued, presented, in progress, completed, skipped, deferred, blocked, or failed.
* This table enables durable planned-vs-completed analytics for daily operator behavior.
...
* `users` -> many `daily_command_queues`
* `daily_command_queues` -> many `command_queue_items`
* `command_queue_items` -> optional one `offerings`
* `command_queue_items` -> optional one `campaigns`
* `command_queue_items` -> optional one `action_lanes`
* `command_queue_items` -> optional one `action_cycles`
* `command_queue_items` -> optional one `action_items`
* Action cycles organize work by campaign/lane/window; command queues organize work by what the user should execute today.
