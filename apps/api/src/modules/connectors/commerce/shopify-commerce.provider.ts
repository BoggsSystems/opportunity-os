...
export class ShopifyCommerceProvider implements CommerceProvider {
  ...
  async listCustomers(credentials: CommerceConnectorCredentials): Promise<CommerceCustomerData[]> {
    const query = `
      query {
        customers(first: 50) {
          edges {
  ...
  async listOrders(credentials: CommerceConnectorCredentials): Promise<CommerceOrderData[]> {
    const query = `
      query {
        orders(first: 50) {
          edges {
  ...
  async listProducts(credentials: CommerceConnectorCredentials): Promise<CommerceProductData[]> {
    const query = `
      query {
        products(first: 50) {
          edges {
  ...
  private async graphql(credentials: CommerceConnectorCredentials, query: string) {
    ...
    if (!response.ok || payload.errors) {
      const detail = payload.errors ? JSON.stringify(payload.errors) : await response.text();
